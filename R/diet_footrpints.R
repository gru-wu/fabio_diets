...
fp_sq_2013_continent <- fp_aggregate(fp_sq_2013, aggregate_by = c("continent_origin"), indicators = c("landuse", "blue", "ghg", "luh", "ghg_all", "biomass"))
...
Y_agg <- Y_food_aut[, lapply(.SD, sum, na.rm=TRUE), by = comm_group_ger, .SDcols = c("food_g_pc_day_net", "eat_g_pc_day_net")]#, "food_kcal_pc_day_net", "eat_kcal_pc_day_net")]
Y_agg_long <- Y_agg %>%
  rename_with(~gsub("_g_pc_day_net", "", .x, fixed = TRUE)) %>%
  pivot_longer(names_to = "diet", cols = c(food:eat), values_to = "g_pc_day") %>%
  filter(g_pc_day > 0) %>%
  mutate(diet_lab = ifelse(diet == "food", "Status Quo", "EAT Lancet"))
...
fp_agg <- as.data.frame(rbind(fp_sq_2013_agg, fp_eat_2013_agg))
fp_agg_land <- as.data.frame(rbind(fp_sq_2013_agg_crop, fp_eat_2013_agg_crop))

fp_agg$diet <- factor(c("Status \nQuo", "EAT \nLancet"), levels = c("Status \nQuo", "EAT \nLancet"))
